...
var next_lesson_button: Button = null
...
func _ready() -> void:
	...
	next_lesson_button = _find_button_by_name(self, "NextLessonButton")
	if next_lesson_button == null:
		next_lesson_button = _find_button_by_name(self, "SummitButton")

	if next_lesson_button != null:
		next_lesson_button.pressed.connect(_on_next_lesson_pressed)
...
func _on_next_lesson_pressed() -> void:
	print("[DEBUG] NEXT LESSON BUTTON CLICKED!")
	if run_button != null:
		run_button.text = "LOADING..."
	var next_level := ""
...
func _setup_tutorial_ghost_text() -> void:
	...
	# ล้าง editor แล้วใส่โค้ดว่าง
	_is_restoring_editor_text = true
	code_editor.text = ""
	_is_restoring_editor_text = false

	# สร้าง RichTextLabel เป็น ghost overlay
	...
	if playground_subtitle_label != null:
		...
		playground_subtitle_label.text = "📝  %s  —  %s" % [cat, desc]
		playground_subtitle_label.add_theme_color_override("font_color", Color8(100, 210, 255))

	_update_ghost_text()
	_print("👻 Ghost text tutorial activated: %s" % mission_level_id)

## อัปเดต ghost text ตามที่เด็กพิมพ์
	...
	for line_idx in range(target_lines.size()):
		...
		for char_idx in range(tgt_len):
			...
			if char_idx < typ_len:
				...
				if char_idx == typ_len:
					result_bbcode += " "
				result_bbcode += "[color=#556677]%s[/color]" % ch
...
func _apply_tutorial_console_style() -> void:
	...
	# แสดงปุ่ม NEXT LESSON สำหรับให้กดไปด่านถัดไป
	if next_lesson_button != null:
		next_lesson_button.visible = true
		next_lesson_button.text = "NEXT LESSON"
...
func _apply_playground_panel_theme() -> void:
	...
	if next_lesson_button != null:
		next_lesson_button.text = "NEXT LESSON"
		next_lesson_button.add_theme_color_override("font_color", Color8(255, 255, 255))
		next_lesson_button.add_theme_font_size_override("font_size", 12)
		next_lesson_button.add_theme_stylebox_override("normal", _make_ui_style(Color8(72, 96, 210), 14, Color8(36, 49, 108), 2))
		next_lesson_button.add_theme_stylebox_override("hover", _make_ui_style(Color8(95, 124, 238), 14, Color8(36, 49, 108), 2))
		next_lesson_button.add_theme_stylebox_override("pressed", _make_ui_style(Color8(50, 72, 158), 14, Color8(36, 49, 108), 2))
...
func _apply_playground_layout() -> void:
	if _is_embedded_split_layout():
		...
		if next_lesson_button != null:
			next_lesson_button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		if panel_expand_button != null:
